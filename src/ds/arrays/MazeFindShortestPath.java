		...
		while (!queue.isEmpty()) {
			...
			for (Cell nextCell : nextCells) {
				...
				if (nextCell.row == maze.length
					...
					continue;
				System.out.print("Tested levelMatrix["+nextCell.row+"]["+nextCell.col+"]:"+levelMatrix[nextCell.row][nextCell.col] + " | ");

				if (levelMatrix[nextCell.row][nextCell.col] == 0) {
					queue.add(nextCell);
					levelMatrix[nextCell.row][nextCell.col] = level + 1;
					System.out.print("levelMatrix["+nextCell.row+"]["+nextCell.col+"]:"+levelMatrix[nextCell.row][nextCell.col] + " | ");
				}
			}
			System.out.println();
